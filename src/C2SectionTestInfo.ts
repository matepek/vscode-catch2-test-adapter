...
export class C2TestInfo implements TestInfo {
  ...
  readonly sectionNameFull: string;
  readonly sectionNameTrimmed: string;
  ...
  constructor(
    ...
    this.sectionNameFull = sectionName;
    this.sectionNameTrimmed = sectionName.trim();

    this.label = sectionName;
    this.id = generateUniqueId();
  }

  getEscapedSectionName(): string {
    // TODO test it
    let t = this.sectionNameFull.replace(",", "\\,").replace("[", "\\[");
    if (t[0] == " ") t = "*" + t.substr(1);
    return t;
  }
