...
import { C2AllTestSuiteInfo, C2TestInfo, C2TestSuiteInfo } from "./catch2";
...
export class Catch2TestAdapter implements TestAdapter, vscode.Disposable {
  ...
  private reloadSuiteChildren(suite: C2TestSuiteInfo): Promise<void> {
    return promisify(fs.exists)(suite.execPath).then((exists: boolean) => {
      if (!exists) throw Error("reloadSuiteChildren: Should exists: " + suite.execPath);
        ...
        execFile(
          ...
          (error: Error | null, stdout: string, stderr: string) => {
            ...
            while (i < lines.length - 1) {
              if (lines[i][0] != " ")
                this.log.error("Wrong test list output format: " + lines.toString());
              ...
              suite.createChildTest(testName, description, tags, filePath, line);
            }
  ...
  private loadSuite(exe: ExecutableConfig): Promise<void> {
    const suite = this.allTests.createChildSuite(exe.name, exe.path, {
      cwd: exe.cwd,
      env: exe.env
    });
    ...
    watcher.on("change", (eventType: string, filename: string) => {
      ...
      const waitAndThenTry = (remainingIteration: number, delay: number) => {
        if (remainingIteration == 0) {
          ...
          setTimeout(waitAndThenTry, delay, [remainingIteration - 1, Math.max(delay * 2, 2000)]);
        } else {
          this.testsEmitter.fire({ type: "started" });
  ...
  load(): Promise<void> {
    ...
    this.allTests = new C2AllTestSuiteInfo(this, this.getWorkerMaxNumber(config));
        ...
        const recursiveAdd = (directory: string): void => {
          ...
          children.forEach(child => {
            ...
            if (childPath.match(regex) && childStat.isFile()) {
              ...
              executables.push(
                new ExecutableConfig(resolvedName, childPath, regex, fullPath(resolvedCwd), env)
              );
  ...
  private filterVerifiedCatch2TestExecutables(
    ...
    executables.forEach(exec => {
      promises.push(
        this.verifyIsCatch2TestExecutable(exec.path).then((isCatch2: boolean) => {
