...
export class C2AllTestSuiteInfo implements TestSuiteInfo {
  ...
  createChildSuite(label: string, execPath: string, execOptions: SpawnOptions): C2TestSuiteInfo {
    const suite = new C2TestSuiteInfo(label, this.adapter, [this.taskPool], execPath, execOptions);
...
export class C2TestSuiteInfo implements TestSuiteInfo {
  ...
  createChildTest(
    ...
    const test = new C2TestInfo(testName, description, tags, file, line - 1, this, this.adapter);
  ...
  private runInner(childrenToRun: C2TestInfo[] | "all"): Promise<void> {
    ...
    const data: {
      [prop: string]: any;
    } = { buffer: "", currentChild: undefined, inTestCase: false };
