...
export class C2ExecutableInfo implements vscode.Disposable {
  ...
  private readonly _executables: Map<string /** filePath */, C2TestSuiteInfo> = new Map();
  ...
  async load(): Promise<void> {
    ...
    if (!isAbsolute || (isAbsolute && isPartOfWs)) {
      ...
      relativePattern = new vscode.RelativePattern(this._adapter.workspaceFolder, relativeToWs);
      fileUris = await vscode.workspace.findFiles(relativePattern, undefined, 1000);
      try {
  ...
  private _addFile(file: vscode.Uri) {
    ...
    const suite = this._allTests.createChildSuite(resolvedName, file.path, {
      cwd: resolvedCwd,
      env: resolvedEnv
    });
  ...
  private readonly _lastEventArrivedAt: Map<
    string /* fsPath */,
    number /** Date.now */
  > = new Map();
  ...
  private _handleEverything(uri: vscode.Uri) {
    ...
    const x = (exists: boolean, timeout: number, delay: number): Promise<void> => {
      ...
      if (Date.now() - lastEventArrivedAt! > timeout) {
        ...
        this._adapter.testsEmitter.fire({ type: "finished", suite: this._allTests });
        return Promise.resolve();
      } else if (exists) {
        this._adapter.testsEmitter.fire({ type: "started" });
