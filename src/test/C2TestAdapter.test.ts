        ...
        const waitForReloadAndAsser = (tryCount: number): Promise<void> => {
          if (testsEvents.length < 2)
            return new Promise<void>(r => setTimeout(r, 10)).then(function() {
              waitForReloadAndAsser(tryCount - 1)
            });
          else {
  ...
  context('example1', function() {
    ...
    describe('load', function() {
      ...
      context('suite1 and suite2 are used', function() {
        ...
        const testsForAdapterWithSuite1AndSuite2:
            [string, Mocha.AsyncFunc|Mocha.Func][] = [
              [
              ...
              [
                ...
                async function(this: Mocha.Context) {
                  this.slow(150);
                  const doAndWaitForReload =
                      async function(action: Function): Promise<TestSuiteInfo> {
                    ...
                    const e = <TestLoadFinishedEvent>
                        testsEvents[testsEvents.length - 1]!;
                  ...
                  {  // touch
                    const newRoot = await doAndWaitForReload(async () => {
                      suite1Watcher.emit(
                  ...
                  {  // test deleted
                    ...
                    const deletedTest = oldSuite1Children[0];
                    assert.equal(deletedTest.label, 's1t0');
