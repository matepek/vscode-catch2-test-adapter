...
describe(
    ...
    function() {
      ...
      before(function() {
        ...
        spawnStub =
            sinonSandbox.stub(child_process, 'spawn').named('spawnStub');
        vsfsWatchStub =
          ...
          context('suite1 and suite2 are used', function() {
            ...
            const testsForAdapterWithSuite1AndSuite2: Mocha.Test[] = [
              ...
              new Mocha.Test(
                  ...
                  async function() {
                    ...
                    assert.ok(
                        testStatesEvI(running) < testStatesEvI(s1running));
                    assert.ok(
                        testStatesEvI(s1running) < testStatesEvI(s1finished));
                    ...
                    assert.ok(
                        testStatesEvI(running) < testStatesEvI(s1running));
                    assert.ok(
                        testStatesEvI(s2running) < testStatesEvI(s2finished));
                    ...
                    assert.ok(
                        testStatesEvI(s1t1running) <
                        testStatesEvI(s1t1finished));
                    assert.ok(
                        testStatesEvI(s1t1finished) <
                        testStatesEvI(s1finished));
                    ...
                    assert.ok(
                        testStatesEvI(s1t2running) <
                        testStatesEvI(s1t2finished));
                    assert.ok(
                        testStatesEvI(s1t2finished) <
                        testStatesEvI(s1finished));
                    ...
                    assert.ok(
                        testStatesEvI(s2t1running) <
                        testStatesEvI(s2t1finished));
                    assert.ok(
                        testStatesEvI(s2t1finished) <
                        testStatesEvI(s2finished));
                    ...
                    assert.ok(
                        testStatesEvI(s2t2running) <
                        testStatesEvI(s2t2finished));
                    assert.ok(
                        testStatesEvI(s2t2finished) <
                        testStatesEvI(s2finished));
                    ...
                    assert.ok(
                        testStatesEvI(s2t3running) <
                        testStatesEvI(s2t3finished));
                    assert.ok(
                        testStatesEvI(s2t3finished) <
                        testStatesEvI(s2finished));
                    ...
                    assert.ok(
                        testStatesEvI(s1finished) < testStatesEvI(finished));
                    assert.ok(
                        testStatesEvI(s2finished) < testStatesEvI(finished));
                  ...
                  async function() {
                    ...
                    withArgs.onCall(withArgs.callCount)
                        .returns(new ChildProcessStub(
                            example1.suite2.t3.outputs[0][1]));
              ...
              new Mocha.Test(
                  ...
                  async function() {
                    const m = example1.suite1.t1.outputs[0][1].match(
                        '<TestCase[^>]+>');
            ...
            context(
                ...
                function() {
                  ...
                  for (let t of testsForAdapterWithSuite1AndSuite2) this
                      .addTest(t.clone());
                  ...
                  it('reload because of fswatcher event: test added',
                     async function(this: Mocha.Context) {
                       ...
                       assert.equal(
                           root.children.length, oldRootChildren.length);
                       for (let i = 0; i < oldRootChildren.length; i++) {
                         assert.equal(root.children[i], oldRootChildren[i]);
                       }

                       assert.equal(
                           suite1.children.length,
                           oldSuite1Children.length + 1);
                       for (let i = 0; i < suite1.children.length; i++) {
                         assert.equal(
                             suite1.children[i + 1], oldSuite1Children[i]);
                       }
                  ...
                  it('reload because of fswatcher event: test deleted',
                     async function(this: Mocha.Context) {
                       ...
                       assert.equal(
                           suite1.children.length + 1,
                           oldSuite1Children.length);
                       for (let i = 0; i < suite1.children.length; i++) {
                         assert.equal(
                             suite1.children[i], oldSuite1Children[i + 1]);
                       }
              ...
              async function() {
                ...
                it('run suite3 one-by-one', async function() {
                  ...
                  const runAndCheckEvents = async (test: TestInfo) => {
                    ...
                    assert.equal(
                        (<TestEvent>testStatesEvents[2]).state, 'running');
                    ...
                    assert.deepStrictEqual(
                        ...
                        testStatesEvents[4]);
                    assert.deepStrictEqual(
                        {type: 'finished'}, testStatesEvents[5]);
    ...
    // TODO tests outside working directory
