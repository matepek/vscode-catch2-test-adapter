...
--------------------------------------------------- | -----------
`catch2TestExplorer.executables`                    | The location of your test executables (relative to the workspace folder or absolute path) and with a lot of other setting. [Details](#catch2TestExplorerexecutables)
...
`catch2TestExplorer.debugConfigTemplate`            | Set the necessary debug configurations and the debug button will work. [Details](#catch2TestExplorerdebugConfigTemplate)
`catch2TestExplorer.debugBreakOnFailure`            | Debugger breaks on failure while debugging the test. Catch2: [--break][catch-break]; Google Test: [--gtest_break_on_failure][gtest-gtest_break_on_failure];
`catch2TestExplorer.defaultNoThrow`                 | Skips all assertions that test that an exception is thrown, e.g. REQUIRE_THROWS. This is a Catch2 parameter: [--nothrow][catch-nothrow]
`catch2TestExplorer.defaultRngSeed`                 | Shuffles the tests with the given random. Catch2: [--rng-seed (<integer> or 'time')][catch-rng-seed]; Google Test: [--gtest_random_seed=<integer>][gtest-gtest_random_seed];
...
`catch2TestExplorer.googletest.treatGmockWarningAs` | Forces the test to be failed even it is passed if contains the string `GMOCK_WARNING:`. (You may should consider using [testing::StrictMock<T>][gtest-strict-mock]
`testExplorer.errorDecoration`                      | Show error messages from test failures as decorations in the editor. [Details][vscode-test-explorer-configuration]
`testExplorer.gutterDecoration`                     | Show the state of each test in the editor using Gutter Decorations. [Details][vscode-test-explorer-configuration]
`testExplorer.codeLens`                             | Show a CodeLens above each test or suite for running or debugging the tests. [Details][vscode-test-explorer-configuration]
`testExplorer.onStart`                              | Retire or reset all test states whenever a test run is started. [Details][vscode-test-explorer-configuration]
`testExplorer.onReload`                             | Retire or reset all test states whenever the test tree is reloaded. [Details][vscode-test-explorer-configuration]
...
[configuration][vscode-test-explorer-configuration] and [commands][vscode-test-explorer-commands]
can be used.
...
`pattern`     | A relative (to workspace directory) or an absolute path or [_glob pattern_](). ⚠️**Avoid backslash!**: 🚫`\`; ✅`/`; (required)
...
`dependsOn`   | Array of (relative / absolute) _paths_ / [_glob pattern_][vscode-advanced-search] (string[]). If a related file is _changed/created/deleted_ and autorun is enabled in "..." menu it will run the related executables.

The `pattern` (or the `executables` used as string or an array of strings)
can contains [_search-pattern_][vscode-advanced-search].
...
<!-- Reference links -->
[catch-break]: 												https://github.com/catchorg/Catch2/blob/master/docs/command-line.md#breaking-into-the-debugger
[catch-rng-seed]: 										https://github.com/catchorg/Catch2/blob/master/docs/command-line.md#rng-seed
[catch-nothrow]: 											https://github.com/catchorg/Catch2/blob/master/docs/command-line.md#eliding-assertions-expected-to-throw
[gtest-gtest_break_on_failure]: 			https://github.com/google/googletest/blob/master/googletest/docs/advanced.md#turning-assertion-failures-into-break-points
[gtest-gtest_random_seed]: 						https://github.com/google/googletest/blob/master/googletest/docs/advanced.md#shuffling-the-tests
[gtest-strict-mock]: 									https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md#the-nice-the-strict-and-the-naggy
[vscode-advanced-search]: 						https://code.visualstudio.com/docs/editor/codebasics#_advanced-search-options
[vscode-test-explorer-configuration]: https://github.com/hbenl/vscode-test-explorer#configuration
[vscode-test-explorer-commands]: 			https://github.com/hbenl/vscode-test-explorer#commands
