...
| `${switch(os)}${win}<W>${mac}<M>${lin}<L>${endswitch}` | `<W>` on Windows, `<M>` on macOS, `<L>` on Linux. You can have 1-3 cases, and the last one can be `def` for default (e.g. `${switch(os)}${win}<W>${def}<D>${endswitch}`). Empty will be used if the running OS is not in a case. |
